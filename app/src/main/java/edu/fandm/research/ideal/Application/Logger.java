...
import edu.fandm.research.ideal.BuildConfig;
...
public class Logger {
    ...
    public static File getDiskCacheDir() {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState()) || !Environment.isExternalStorageRemovable()) {
            cacheDir = IDEAL.getAppContext().getExternalCacheDir();
        }
        if (cacheDir == null) {
            if (BuildConfig.DEBUG) {
                Log.d("LoggerManager", "External Cache Directory not available.");
            }
            cacheDir = IDEAL.getAppContext().getCacheDir();
        }
        if (BuildConfig.DEBUG) {
    ...
    public static File getDiskFileDir() {
        ...
        if (BuildConfig.DEBUG && (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState()) || !Environment.isExternalStorageRemovable())) {
            fileDir = IDEAL.getAppContext().getExternalFilesDir(null);
        }
        if (fileDir == null) {
            if (BuildConfig.DEBUG) {
                Log.d("LoggerManager", "External Cache Directory not available.");
            }
            fileDir = IDEAL.getAppContext().getFilesDir();
        }
        if (BuildConfig.DEBUG) {
